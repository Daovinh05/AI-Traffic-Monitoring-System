...
def driver_monitor(vehicle_id=None):
    ...
    try:
        ...
        timing_stats = {
            "face_detection": [], "landmark_predictor": [], "ear_computation": [],
            "yawn_detection": [], "head_pose": [], "phone_detection": [],
            "seatbelt_detection": [], "hand_detection": [], "total_frame": []
        }
        frame_count = 0
        LOG_INTERVAL = 100  # Log mỗi 100 frame
        ...
        while active_video_stream == 'driver':
            ...
            try:
                ...
                faces = []
            timing_stats["face_detection"].append(time.perf_counter() - t0)
            ...
            for face in faces:
                ...
                timing_stats["landmark_predictor"].append(time.perf_counter() - t0)
                ...
                if warning_states["eye"]:
                    ...
                    if ear < EAR_THRESHOLD:
                        ...
                        eye_closed_time = None
                    timing_stats["ear_computation"].append(time.perf_counter() - t0)
                ...
                if warning_states["yawn"]:
                    ...
                    if yawn_ratio > YAWN_THRESHOLD:
                        ...
                        counter_yawn = 0
                        alarm_yawn_on = False
                    timing_stats["yawn_detection"].append(time.perf_counter() - t0)
                ...
                if warning_states["head"]:
                    ...
                    cv2.putText(frame, f"Roll: {roll:.1f}", (10, 70),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                    timing_stats["head_pose"].append(time.perf_counter() - t0)
            ...
            if warning_states["phone"]:
                ...
                timing_stats["phone_detection"].append(time.perf_counter() - t0)
            ...
            if warning_states["seatbelt"]:
                ...
                timing_stats["seatbelt_detection"].append(time.perf_counter() - t0)
            ...
            if warning_states["hand"]:
                ...
                timing_stats["hand_detection"].append(time.perf_counter() - t0)
            ...
            timing_stats["total_frame"].append(time.perf_counter() - frame_start)
            frame_count += 1
            ...
            if frame_count % LOG_INTERVAL == 0:
                ...
                for key, values in timing_stats.items():
                    if values:
                        ...
                        fps = 1.0 / (sum(values) / len(values)) if sum(values) > 0 else 0
                        print(f"  {key:25s}: avg={avg_ms:.3f}ms  fps={fps:.2f}")
                total_avg = (sum(timing_stats["total_frame"]) / len(timing_stats["total_frame"])) * 1000
                print(f"  {'---TOTAL PIPELINE---':25s}: avg={total_avg:.3f}ms  fps={1000/total_avg:.2f}")
                print("=" * 50)

            yield (b'--frame\r\nContent-Type: image/jpeg\r\n\r\n' + frame + b'\r\n')

        # --- Final summary ---
        print(f"\n===== BENCHMARK FINAL ({frame_count} total frames) =====")
        for key, values in timing_stats.items():
            if values:
                avg_ms = (sum(values) / len(values)) * 1000
                fps = 1.0 / (sum(values) / len(values)) if sum(values) > 0 else 0
                print(f"  {key:25s}: avg={avg_ms:.3f}ms  fps={fps:.2f}")
        if timing_stats["total_frame"]:
            total_avg = (sum(timing_stats["total_frame"]) / len(timing_stats["total_frame"])) * 1000
            print(f"  {'---TOTAL PIPELINE---':25s}: avg={total_avg:.3f}ms  fps={1000/total_avg:.2f}")
        print("=" * 50)
    except Exception as e:
