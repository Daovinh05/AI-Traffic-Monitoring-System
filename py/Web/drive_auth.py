...
REACT_INDEX_PATH = os.path.join(app.static_folder, 'react-spa', 'index.html')
...
def should_use_react_frontend():
    """Enable React SPA only when a built bundle is available."""
    return os.path.exists(REACT_INDEX_PATH)
...
def to_json_safe(data):
    """Convert non-JSON-native values (e.g. Decimal, datetime) to strings."""
...
def build_dashboard_context(page, per_page=5):
    conn = get_db_connection()
    if not conn:
        raise RuntimeError('Lỗi kết nối CSDL')
    ...
    cur.execute('''
        ...
        FROM phuong_tien
    ''')
    all_vehicles_for_stats = cur.fetchall()

    stats = {
        'total_vehicles': len(all_vehicles_for_stats),
        'running': sum(1 for v in all_vehicles_for_stats if v['status'] == 'Đang chạy'),
        'stopped': sum(1 for v in all_vehicles_for_stats if v['status'] == 'Đang dừng'),
        'offline': sum(1 for v in all_vehicles_for_stats if v['status'] == 'Mất tín hiệu'),
        'alerts': 0,
        'violations': 0,
        'quality': 86.0
    }
    ...
    # Lấy toàn bộ xe không phân trang cho dashboard
    cur.execute('''
        SELECT p.id, p.bien_so as plate_number, p.loai_xe as type, p.hinh_anh_xe as image,
               t.ho_ten as driver_name, t.so_dien_thoai as phone, t.diem_danh_gia as score,
               t.anh_dai_dien as driver_image,
...
def render_legacy_dashboard():
    try:
        ...
        context = build_dashboard_context(page=page, per_page=5)
...
def api_dashboard_data():
    try:
        page = request.args.get('page', 1, type=int)
        context = build_dashboard_context(page=page, per_page=5)
        return jsonify({'success': True, 'data': to_json_safe(context)}), 200
    except Exception as e:
        return jsonify({'success': False, 'message': f'Lỗi: {str(e)}'}), 500
...
def trang_chu_page():
    ...
    return render_legacy_trang_chu()


@app.route('/legacy/trang_chu')
@login_required
def legacy_trang_chu_page():
    return render_legacy_trang_chu()


def render_legacy_trang_chu():
...
def tu_van_page():
    ...
    return render_legacy_tu_van()


@app.route('/legacy/tu_van')
@login_required
def legacy_tu_van_page():
    return render_legacy_tu_van()


def render_legacy_tu_van():
...
def lai_xe_page():
    ...
    return render_legacy_lai_xe()


@app.route('/legacy/lai_xe')
@login_required
def legacy_lai_xe_page():
    return render_legacy_lai_xe()


def render_legacy_lai_xe():
...
def lich_su_page():
    ...
    return render_legacy_lich_su()


@app.route('/legacy/lich_su')
@login_required
def legacy_lich_su_page():
    return render_legacy_lich_su()


def render_legacy_lich_su():
...
def lai_xe_v2_page():
    if should_use_react_frontend():
        response = make_response(serve_react_frontend())
    else:
...
@login_required
def stop_camera():
    """Stop active monitoring stream safely (legacy compatibility)."""
    global active_video_stream, video_capture
    try:
        active_video_stream = None
...
@app.route('/api/set_monitoring_vehicle_public', methods=['POST'])
def api_set_monitoring_vehicle_public():
    """Public compatibility alias for setting monitored vehicle ID."""
