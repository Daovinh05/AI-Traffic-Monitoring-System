...
# Per-user browser camera state (avoid cross-user warning collision)
browser_driver_states = {}
browser_driver_states_lock = threading.Lock()
BROWSER_DRIVER_STATE_TTL_SECONDS = 15 * 60
driver_ai_lock = threading.Lock()
BROWSER_YAWN_MIN_DURATION = 1.2


def _default_driver_warning_states():
    return {
        'eye': True,
        'yawn': True,
        'head': True,
...
def _cleanup_browser_driver_states(now_ts=None):
    """Remove stale browser driver states to avoid memory growth."""
    if now_ts is None:
        now_ts = time.time()

    with browser_driver_states_lock:
        stale_user_ids = [
            uid for uid, state in browser_driver_states.items()
            if now_ts - state.get('last_seen', 0) > BROWSER_DRIVER_STATE_TTL_SECONDS
        ]
        for uid in stale_user_ids:
            browser_driver_states.pop(uid, None)


def _get_or_create_browser_state(user_id):
    """Get per-user driver state and refresh last_seen timestamp."""
    now_ts = time.time()
    with browser_driver_states_lock:
        state = browser_driver_states.setdefault(user_id, {
            'previous': {
                'eye': '',
                'yawn': '',
                'head': '',
            ...
            'alarm_yawn_on': False,
            'warning_states': _default_driver_warning_states(),
            'last_seen': now_ts
        })
        state['last_seen'] = now_ts
        if 'warning_states' not in state:
            state['warning_states'] = _default_driver_warning_states()
        return state
...
def api_logout():
    ...
    if user_id is not None:
        with browser_driver_states_lock:
            browser_driver_states.pop(user_id, None)
    session.clear()
...
def toggle_warning():
    ...
    try:
        ...
        # Driver warnings are per-user in browser-camera mode.
        if warning_type in ('eye', 'yawn', 'head', 'phone', 'seatbelt', 'hand'):
            user_id = session.get('user_id')
            if user_id is not None:
                state = _get_or_create_browser_state(user_id)
                with browser_driver_states_lock:
                    state = browser_driver_states.setdefault(user_id, state)
                    user_warning_states = state.setdefault('warning_states', _default_driver_warning_states())
                    user_warning_states[warning_type] = bool(enabled)
...
@login_required
def get_warnings():
    _cleanup_browser_driver_states()
    warnings_with_image = warnings.copy()

    user_id = session.get('user_id')
    if user_id is not None:
        with browser_driver_states_lock:
            state = browser_driver_states.get(user_id)
            if state and state.get('latest'):
                for key in ('eye', 'yawn', 'head', 'phone', 'seatbelt', 'hand'):
                    warnings_with_image[key] = state['latest'].get(key, '')
...
def set_mode():
    ...
    try:
        ...
        # Entering driver mode should behave like initial defaults for this user.
        if new_mode == 'driver':
            user_id = session.get('user_id')
            if user_id is not None:
                with browser_driver_states_lock:
                    state = browser_driver_states.setdefault(user_id, {
                        'previous': {'eye': '', 'yawn': '', 'head': '', 'phone': '', 'seatbelt': '', 'hand': ''},
                        'latest': {'eye': '', 'yawn': '', 'head': '', 'phone': '', 'seatbelt': '', 'hand': ''},
                        'eye_closed_time': None,
                        'counter_yawn': 0,
                        'yawn_start_time': None,
                        'alarm_yawn_on': False,
                    ...
                    state['warning_states'] = _default_driver_warning_states()
                    state['eye_closed_time'] = None
                    state['counter_yawn'] = 0
                    state['yawn_start_time'] = None
                    state['alarm_yawn_on'] = False
...
@app.route('/api/process_driver_frame', methods=['POST'])
@login_required
def api_process_driver_frame():
    """Nhận frame từ camera browser của user và xử lý AI theo phiên đăng nhập"""
    try:
        data = request.get_json(silent=True) or {}
        frame_data = data.get('frame', '')
        if not frame_data:
            return jsonify({'success': False, 'message': 'Thiếu frame'}), 400
        ...
        try:
            ...
            return jsonify({'success': False, 'message': 'Frame base64 không hợp lệ'}), 400

        nparr = np.frombuffer(image_bytes, np.uint8)
        frame = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
        if frame is None:
            return jsonify({'success': False, 'message': 'Không decode được frame'}), 400

        _cleanup_browser_driver_states()
        vehicle_id = session.get('vehicle_id')
        user_id = session.get('user_id')

        # Lấy state theo user để giữ logic thời gian liên tục như pipeline cũ.
        state = _get_or_create_browser_state(user_id)
        user_previous_warnings = state['previous']
        user_warning_states = state.get('warning_states', _default_driver_warning_states())
        eye_closed_time = state.get('eye_closed_time')
        counter_yawn = state.get('counter_yawn', 0)
        yawn_start_time = state.get('yawn_start_time')
        alarm_yawn_on = state.get('alarm_yawn_on', False)

        # Copy để tránh sửa trực tiếp tham chiếu toàn cục
        local_warnings = {
            'eye': '',
            'yawn': '',
            'head': '',
        ...
        with driver_ai_lock:
            ...
            for face in faces:
                ...
                # Vẽ đầy đủ landmark khuôn mặt để giống giao diện gốc.
                for i in range(0, 16):
                ...
                if user_warning_states.get('eye', True):
                    ...
                    if ear < EAR_THRESHOLD:
                        if eye_closed_time is None:
                            ...
                            local_warnings['eye'] = 'NHẮM MẮT QUÁ LÂU!'
                            if user_previous_warnings.get('eye') != local_warnings['eye']:
                                add_ai_alert('eye', 'Tài xế đang nhắm mắt quá lâu!', vehicle_id)
                    else:
                        eye_closed_time = None

                if user_warning_states.get('yawn', True):
                    yawn_ratio = detect_yawn(points)
                    if yawn_ratio > YAWN_THRESHOLD:
                        if yawn_start_time is None:
                            yawn_start_time = time.time()
                        elif time.time() - yawn_start_time >= BROWSER_YAWN_MIN_DURATION:
                            if not alarm_yawn_on:
                                alarm_yawn_on = True
                            local_warnings['yawn'] = 'NGÁP NGỦ!'
                            if user_previous_warnings.get('yawn') != local_warnings['yawn']:
                ...
                if user_warning_states.get('head', True):
                    pitch, yaw, _roll = get_head_pose(points, frame.shape)
                    if abs(yaw) > 40 or pitch > 35:
                        local_warnings['head'] = 'MẤT TẬP TRUNG !'
                        if user_previous_warnings.get('head') != local_warnings['head']:
                            add_ai_alert('head', 'Tài xế mất tập trung (quay đầu/ngửa đầu)!', vehicle_id)

                    cv2.rectangle(frame, (10, 10), (220, 90), (0, 0, 0), cv2.FILLED)
                    cv2.putText(frame, f'Pitch: {pitch:.1f}', (15, 35),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                    cv2.putText(frame, f'Yaw: {yaw:.1f}', (15, 58),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)

            # Phone detection
            if user_warning_states.get('phone', True):
                phone_results = phone_mau(frame)
                for result in phone_results:
                    for box in result.boxes.data:
                        x1, y1, x2, y2, conf, cls = box
                        label = phone_mau.names[int(cls)]
                        if "phone" in label.lower() and conf > 0.5:
                            local_warnings['phone'] = 'DÙNG ĐIỆN THOẠI!'
                            if user_previous_warnings.get('phone') != local_warnings['phone']:
            ...
            # Seatbelt detection
            if user_warning_states.get('seatbelt', True):
                ...
                if not seatbelt_detected:
                    local_warnings['seatbelt'] = 'KHÔNG ĐEO DÂY AN TOÀN!'
                    if user_previous_warnings.get('seatbelt') != local_warnings['seatbelt']:
                        add_ai_alert('seatbelt', 'Tài xế không đeo dây an toàn!', vehicle_id)

            # Hand detection
            if user_warning_states.get('hand', True):
                frame = hand_detector.findArmsAndHands(frame)
                if warnings.get('hand'):
                    local_warnings['hand'] = warnings.get('hand', '')
        if local_warnings['hand'] and user_previous_warnings.get('hand') != local_warnings['hand']:
            add_ai_alert('hand', local_warnings['hand'], vehicle_id)

        with browser_driver_states_lock:
            state = browser_driver_states.setdefault(user_id, {'previous': {}, 'latest': {}, 'last_seen': time.time()})
            state['previous'] = dict(local_warnings)
            state['latest'] = dict(local_warnings)
            state['eye_closed_time'] = eye_closed_time
            state['counter_yawn'] = counter_yawn
            state['yawn_start_time'] = yawn_start_time
            state['alarm_yawn_on'] = alarm_yawn_on
            state['warning_states'] = dict(user_warning_states)
            state['last_seen'] = time.time()

        # Đồng bộ tối thiểu để UI cũ không lỗi nếu còn gọi /get_warnings
        warnings['eye'] = local_warnings['eye']
        warnings['yawn'] = local_warnings['yawn']
        warnings['head'] = local_warnings['head']
        warnings['phone'] = local_warnings['phone']
        warnings['seatbelt'] = local_warnings['seatbelt']
        warnings['hand'] = local_warnings['hand']

        frame = cv2.resize(frame, (frame_width, frame_height))
        ret, buffer = cv2.imencode('.jpg', frame)
        if not ret:
            return jsonify({'success': False, 'message': 'Encode frame thất bại'}), 500
        ...
        return jsonify({'success': False, 'message': f'Lỗi xử lý frame: {str(e)}'}), 500
