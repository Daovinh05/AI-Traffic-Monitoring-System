...
# macOS-specific volume/brightness control (replace pycaw which is Windows-only)
def set_mac_volume(level):
    """Set system volume on macOS (0-100)"""
    try:
        # Convert 0-100 to 0-1 for osascript
        vol = level / 100.0
        subprocess.run(['osascript', '-e', f'set volume output volume {int(vol * 100)}'], 
                      capture_output=True)
        return True
...
def set_mac_brightness(level):
    """Set screen brightness on macOS using brightness command"""
    try:
        # Try using brightness command (requires: brew install brightness)
        subprocess.run(['brightness', '-l', str(level / 100.0)], capture_output=True)
        return True
    except:
        try:
            # Alternative: try using osascript for display brightness
            subprocess.run(['osascript', '-e', f'tell application "System Events" to set display brightness to {level / 100.0}'], 
                          capture_output=True)
            return True
        except Exception as e:
...
try:
    volPer = get_mac_volume()
except:
    volPer = 50
brightnessPer = 50  # Default brightness
                ...
                while not voice_queue.empty():
                    ...
                    if response:
                        ...
                        if "âm thanh" in response:
                            volBar = np.interp(volPer, [0, 100], [400, 150])
                            set_mac_volume(volPer)
                        # Cập nhật độ sáng
                        elif "độ sáng" in response:
                            brightBar = np.interp(brightnessPer, [0, 100], [400, 150])
                            set_mac_brightness(brightnessPer)
                ...
                if hands:
                    for hand in hands:
                        ...
                        if hand_type == "Left":  # Tay trái - điều khiển âm thanh và ánh sáng
                            if len(lmList) != 0:
                                try:
                                    # Ưu tiên ngón trỏ, nếu ngón trỏ giơ lên thì không xử lý ngón giữa
                                    ...
                                    elif fingers[2] and not fingers[1]:
