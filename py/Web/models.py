...
def traffic_sign_monitor(vehicle_id=None):
    ...
    try:
        ...
        while active_video_stream == 'sign':
            ...
            # Thêm delay để duy trì FPS cho file video
            time.sleep(0.04) 
    except Exception as e:
...
def collision_monitor(vehicle_id=None):
    ...
    try:
        ...
        cap = cv2.VideoCapture("py/video_input/o_voi.mp4")
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
        cap.set(cv2.CAP_PROP_FPS, 20)
        ...
        while active_video_stream == 'vacham':
            ret, frame = cap.read()
            if not ret:
                print("Failed to grab frame from collision camera - Restarting video")
                cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                time.sleep(0.1) # Tránh lặp vô hạn quá nhanh
                continue
            ...
            # Thêm delay nhỏ để giảm tải CPU và duy trì FPS (~25-30 FPS)
            time.sleep(0.03)
    except Exception as e:
        ...
        while active_video_stream == 'traffic':
            ...
            if counter:
                try:
                    ...
                    # Duy trì frame rate cho video lưu lượng
                    time.sleep(0.03)
                except Exception as e:
