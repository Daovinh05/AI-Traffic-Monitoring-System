...
def get_driver_camera_source(vehicle_id=None):
    """Resolve driver camera source for each vehicle from DB, fallback to server webcam."""
    if not vehicle_id:
        return 0
    ...
    try:
        conn = pymysql.connect(
            ...
            database=os.environ.get('MYSQL_DB', 'giam_sat'),
            cursorclass=pymysql.cursors.DictCursor,
            charset='utf8mb4'
        )
        cur = conn.cursor()
        ...
        if row and row.get('video_file'):
            video_path = os.path.join('py', 'video_input', row['video_file'])
            if os.path.exists(video_path):
                return video_path
            print(f"Driver video file not found for vehicle {vehicle_id}: {video_path}")
    except Exception as e:
        print(f"Error resolving driver camera source for vehicle {vehicle_id}: {e}")
    finally:
        if cur:
            cur.close()
        if conn:
...
def get_vehicle_camera_source(vehicle_id=None, position='truoc', fallback_path=None):
    """Resolve camera source by vehicle and camera position from DB."""
    if not vehicle_id:
    ...
    try:
        ...
        if row and row.get('video_file'):
            ...
            if os.path.exists(video_path):
                return video_path
            print(f"Video file not found for vehicle {vehicle_id} ({position}): {video_path}")
    except Exception as e:
        print(f"Error resolving camera source for vehicle {vehicle_id} ({position}): {e}")
    finally:
        if cur:
            cur.close()
        if conn:
...
def driver_monitor(vehicle_id=None):
    ...
    global latest_warning, warnings, video_writer, is_recording, warning_states
    cap = None
    try:
        source = get_driver_camera_source(vehicle_id)
        cap = cv2.VideoCapture(source)
        if not cap.isOpened() and source != 0:
            print(f"Failed to open driver source '{source}', fallback to webcam")
            cap.release()
            source = 0
            cap = cv2.VideoCapture(source)

        if not cap.isOpened():
            print("Failed to open driver camera source")
            return
        ...
        while True:
            ...
            if not success:
                if is_file_source:
                    # Loop the sample file for continuous stream per user session.
...
def traffic_sign_monitor(vehicle_id=None):
    ...
    global warnings, video_writer, is_recording, latest_sign_image_path, latest_sign_label
    cap = None
    try:
        source = get_vehicle_camera_source(vehicle_id, position='truoc', fallback_path="py/video_input/bien_bao.mp4")
        cap = cv2.VideoCapture(source)
        if not cap.isOpened():
            print(f"Failed to open traffic sign source: {source}")
            return
...
def collision_monitor(vehicle_id=None):
    ...
    global warnings, video_writer, is_recording, last_collision_warning
    cap = None
    try:
        source = get_vehicle_camera_source(vehicle_id, position='truoc', fallback_path="py/video_input/lech_lan.mp4")
        cap = cv2.VideoCapture(source)
        if not cap.isOpened():
            print(f"Failed to open collision source: {source}")
            return
