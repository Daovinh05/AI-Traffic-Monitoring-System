...
Bảng~\ref{tab:inference} trình bày thời gian suy luận và độ chính xác phân loại của từng mô-đun trong kiểm thử lab trên CPU với độ phân giải 640$\times$480. Bảng này chỉ đo chi tiết trên CPU nhằm chỉ ra nút thắt cổ chai; trên GPU không đo bóc tách từng module mà chỉ đo end-to-end ở Bảng~\ref{tab:comparison} để so sánh trực tiếp. Pipeline xử lý tuần tự trên CPU đạt 2,16 FPS (462,37 ms/frame), trong đó các mô hình YOLOv8 chiếm phần lớn thời gian xử lý.

\vspace{6pt}
\noindent
\begin{minipage}{\columnwidth}
\centering
\captionof{table}{Hiệu năng suy luận và độ chính xác phân loại của các mô-đun trong kiểm thử lab trên CPU (giá trị toàn hệ thống 89,9\% là trung bình các module trong điều kiện lab, khác với độ chính xác end-to-end ở Bảng~\ref{tab:comparison})}
\label{tab:inference}
\fontsize{7}{8.5}\selectfont
\setlength{\tabcolsep}{2.5pt}
\resizebox{\columnwidth}{!}{%
...
Bảng~\ref{tab:comparison} là phép đo khác: độ chính xác cảnh báo end-to-end trên luồng webcam trực tiếp. Trên GPU, hệ thống đạt tốc độ 20--30 FPS với độ trễ 100--200 ms và độ chính xác end-to-end khoảng 92\%, trong khi trên CPU chỉ đạt 2--5 FPS với độ trễ 200--500 ms và khoảng 85\% (Bảng~\ref{tab:comparison}). GPU nhanh hơn 6--10 lần, khẳng định vai trò thiết yếu của tăng tốc phần cứng trong các hệ thống DMS đa module. Như vậy con số 92\% trong tóm tắt là kết quả end-to-end trên GPU, không mâu thuẫn với 89,9\% trung bình module trong kiểm thử lab trên CPU.
